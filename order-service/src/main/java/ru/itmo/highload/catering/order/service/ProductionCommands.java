...
public class ProductionCommands {
    ...
    @Transactional
    public OrderResponse execute(UUID orderId, ProductionCommand command) {
        ...
        var receipts = jdbc.queryForList("select * from production_command where command_id = ?", id);
        if (!receipts.isEmpty()) {
        ...
        try {
            jdbc.update("""
                    insert into production_command
