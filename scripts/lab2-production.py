#!/usr/bin/env python3
"""Exercise production routing, idempotency and dependency failures on a disposable Compose project."""
...
def api(method, path, data=None, expected=200):
    request = urllib.request.Request(base + "/api/v1" + path, method=method,
        ...
        headers={"Content-Type": "application/json", "X-Trace-Id": "production-compose"})
    try:
    ...
    with response:
        ...
        assert response.headers["X-Trace-Id"] == "production-compose"
        return body


def internal(path="/internal/v1/production/tasks?size=50"):
    return json.loads(subprocess.check_output(compose + ["exec", "-T", "config-service", "wget", "-qO-",
        "http://production-service:8080" + path]))
...
def confirmed():
    order = api("POST", "/orders", {"organizationId": org["id"], "deliveryPointId": point["id"],
        "requestedDeliveryAt": "2099-01-01T12:00:00Z", "comment": "Проверка производства"}, 201)
...
internal()
assert sql("production-postgres", f"select status from production_task where order_id='{cancelled['id']}'") == "CANCELLED"

# Production is the only public cooking path; its absence cannot mutate Order.
...
for attempt in range(30):
    ...
    raise AssertionError("Production route did not recover")
...
try:
    ...
    result = subprocess.run(compose + ["exec", "-T", "config-service", "wget", "-SO-",
        "http://production-service:8080/internal/v1/production/tasks"], capture_output=True, text=True)
    assert result.returncode != 0 and "503" in result.stderr, result
    assert sql("production-postgres", f"select status from production_task where order_id='{order['id']}'") == "CONFIRMED"
finally:
    start("order-service")
...
assert sql("production-postgres", f"select status from production_task where order_id='{order['id']}'") == "COMPLETED"
assert sql("order-postgres", f"select count(*) from production_command where order_id='{order['id']}'") == "3"
assert len(api("GET", f"/orders/{order['id']}/history")["items"]) == 5
assert completed["totalAmount"] == 360
print("PASS: Production routing, cancellation, concurrent replay, Order outage and recovery")
