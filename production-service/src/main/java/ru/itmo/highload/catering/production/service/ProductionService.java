...
public class ProductionService {
    ...
    public OrderResponse execute(UUID id, long expectedVersion, ProductionAction action) {
        // Read the owner even on retry, but let its receipt resolve an already executed command.
        var current = orders.get(id);
        UUID commandId = UUID.nameUUIDFromBytes(("production:v1:" + id + ":" + action + ":" + expectedVersion)
                .getBytes(StandardCharsets.UTF_8));
        var result = orders.command(id, new ProductionCommand(commandId, action.expectedStatus(), expectedVersion, action));
